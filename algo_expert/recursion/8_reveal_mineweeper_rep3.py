...
def revealMinesweeper(board, row, column):
    if board[row][column] == 'M':
        board[row][column] = "X"
        return board
    return solve_partial_mines_weeper(board, row, column)


def solve_partial_mines_weeper(board, row, column):

    neighbors = get_neighbors(board, row, column)
    nb_adjacent_mines = 0
    for neig_row, neig_column in neighbors:
        if board[neig_row][neig_column] == "M":
            nb_adjacent_mines += 1
    ...
    board[row][column] = "0"
    for neig_row, neig_column in neighbors:
        if board[neig_row][neig_column] == "H":
            solve_partial_mines_weeper(board, neig_row, neig_column)
...
def get_neighbors(board, row, column):
    ...
    neighbors = []
    for delta_x, delta_y in potential_steps:
        new_row = row + delta_x
